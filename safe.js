...
Safe.validate = function(docOrMod, simpleSchema, options) {
  ...
  check(options, {
    isModifier: Boolean, // Is docOrMod a modifier?
  ...
  if (_.isFunction(simpleSchema.prepare)) {
    ...
    check(doc, Object);
    check(params, {
      isUpdate: Boolean, // Are we doing an update?
      modifier: Match.Optional(Object) // If we have a modifier, it's in here
    });
  ...
  if (! options.context.validate(docOrMod, {modifier: options.isModifier})) {
    ...
    _.each(options.context.getErrorObject().invalidKeys, 
      function(x) { errors[x.name] = x.message; });
 ...
 */
Safe.subscribe = function(/* arguments */) {
...
Safe.Match.WhitelistedObject = function(allowedKeys) {
  return Match.Where(function (x) {
    ...
    _.each(allowedKeys, 
      function(key) { schema[key] = Match.Optional(Match.Any); });
